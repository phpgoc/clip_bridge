...
- WebSocket 只用于 `/server` 上的 WebRTC 信令交换。
...
Clip Bridge 会启动自己的 UDP TURN 服务。浏览器从 `/server/config` 拿到的是 `turn:<current-host>:<turn-port>`，所以 TURN hostname 会跟随当前网页 hostname。
