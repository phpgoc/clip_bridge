...
P2P Clip Bridge Server 是一个小型 Rust 服务，用同一个 URL 房间在多台设备之间传递剪贴板文字和文件。

- 房间密码就是 URL 路径，例如 `/my-room`。
- 剪贴板文字和文件内容通过 WebRTC DataChannel 传输。
- WebSocket 只用于 `/clip_bridge_server` 上的 WebRTC 信令交换。
- 服务端不保存剪贴板文字，也不保存文件内容，后来加入房间的人不会收到之前传过的文字或文件 offer。
...
P2P Clip Bridge Server 永远监听 `0.0.0.0`，启动时只需要选择端口和 TURN 凭据：
...
P2P Clip Bridge Server 会启动自己的 UDP TURN 服务。浏览器从 `/clip_bridge_server/config` 拿到的是 `turn:<current-host>:<turn-port>`，所以 TURN hostname 会跟随当前网页 hostname。
...
剪贴板自动读写只能在浏览器安全上下文中使用，也就是 HTTPS 或 localhost。如果前面使用 nginx 反向代理并且用户打开的是 `https://` 地址，浏览器会识别为安全上下文，页面会继续使用原来的点击剪贴板流程。

如果是非 localhost 的普通 HTTP 地址，文件传输仍然可以正常工作，但浏览器会阻止剪贴板读写权限。此时页面会显示一个手动文字输入框用于发送文字；收到的文字可以点击后全选，用户再手动复制。

WebRTC 每次发送文字或文件 offer 时，只会发给当时已经连接在房间里的 peer。后来加入房间的人不会从服务端或浏览器历史中得到之前传递过的文字或文件数据。
